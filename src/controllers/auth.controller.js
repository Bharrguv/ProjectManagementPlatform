...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (!createdUser) {
    throw new ApiError(500, "Something went wrong while regrestering a user");
  }
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        { user: createdUser },
        "User registered successfully and verification email has been sent on email",
      ),
    );
});
...
const getCurrentUser = asyncHandler(async (req, res) => {
  return res
    .status(200)
    .json(new ApiResponse(200, req.user, "Ccurrent user fetched successfully"));
});
